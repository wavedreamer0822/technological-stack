...
void seclect() //种群选择
{
		...
		for (int j = 0; j < Po_Size; j++)
	{
		...
		{
			if (array[j] < nowpopulation[0].GetSumFitness())
			{
...
void crossing()//杂交
{
	...
	srand((unsigned)time(NULL));//设置随机数种子 设置一次随机种子就行
	double array1[De_Variable], array2[De_Variable];//临时存储父亲和母亲的变量值
	while (num < Po_Size - 1)//个体1与个体2杂交 个体3与个体四杂交 个体i和i+1杂交
	{
		...
		if (corss <= Ov_Probability)//如果corss小于等于杂交概率Ov_Probability就进行单点杂交
		{
			...
			int localx1, localx2;//记录基因交叉点位置
			int corssx1[length1], corssx2[length2];//作为交换基因的数组
			double newx1[2], newx2[2];//分别用来保存基因交换后对应的自变量值
			...
			for (int j = 0; j < De_Variable; j++)//array1的x1编码之后和array2的x1编码后进行单点杂交 以此类推
			{
				if (j == 0)//x1进行编码并且杂交
				{
					bitset<length1> array1b1((array1[j] + 3.0) * pow(10, 6));//加上3之后形成一个unsigned数之后在进行母体1的x1编码
					bitset<length1> array2b1((array2[j] + 3.0) * pow(10, 6));//加上3之后形成一个unsigned数之后在进行母体2的x1编码
				...
				if (j == 1)//进行编码并且杂交
				{
					...
					bitset<length2>array2b2((array2[j]) * pow(10, 6));//母体1的x2编码
